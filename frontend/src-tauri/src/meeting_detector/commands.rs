use super::detector::{
    DetectedMeeting, MeetingDetectionSettings, MeetingDetectionStatus, MeetingDetector,
};
use log::info;
use std::sync::Arc;
...
pub async fn set_meeting_detection_settings<R: Runtime>(
    ...
    match (was_enabled, now_enabled) {
        (false, true) => detector.start_monitoring(app).await,
        (true, false) => detector.stop_monitoring(),
        _ => {}
    }
...
pub async fn check_for_active_meeting(
    state: State<'_, MeetingDetectorState>,
) -> Result<Option<DetectedMeeting>, String> {
...
#[tauri::command]
pub async fn stop_meeting_monitor(state: State<'_, MeetingDetectorState>) -> Result<(), String> {
    let detector = state.read().await;
    detector.stop_monitoring();
    Ok(())
}
