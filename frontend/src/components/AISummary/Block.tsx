  ...
  return (
    <div 
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-blue-50 ring-1 ring-blue-200 shadow-sm dark:bg-blue-950/40 dark:ring-blue-900' : 'hover:bg-gray-50 dark:hover:bg-gray-900'}`}
    ...
    >
      ...
      <div className="relative flex-1 py-0.5 px-1">
        ...
        {showCommands && (
          <div 
            ref={commandsRef}
            className="absolute left-0 top-full mt-1 w-64 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 py-2 z-50
          ...
          >
            {filteredCommands.map((cmd, index) => (
              <button
                key={cmd.id}
                className={`
                  w-full text-left px-3 py-2 flex items-center space-x-3 hover:bg-gray-50 dark:hover:bg-gray-800
                  ${index === selectedCommandIndex ? 'bg-gray-50 dark:bg-gray-800' : ''}
                `}
                onClick={() => handleCommandSelect(cmd)}
                onMouseEnter={() => setSelectedCommandIndex(index)}
              >
                <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center bg-gray-100 dark:bg-gray-800 rounded text-gray-600 dark:text-gray-300">
                  {cmd.icon}
                </span>
